...
**Skills** are procedural knowledge loaded on demand. Create `SKILL.md` files in `~/.jarvis/skills/` with YAML frontmatter and markdown instructions. JARVIS discovers skills at boot, shows a catalog in the system prompt, and loads full content only when invoked — via AI auto-invoke (trigger matching), slash commands, or direct tool call. Skills support per-session isolation (each actor has independent active skills), argument substitution, shell preprocessing, token budgets, and `context: fork` for dispatching to isolated actors. Install the [skills plugin](https://github.com/giovanibarili/jarvis-plugin-skills) to enable.
